...
bool WifiManager::autoReconnectPaused_ = false;
...
bool WifiManager::connect(const char* ssid, const char* pass, bool save) {
  ...
  autoReconnectPaused_ = false;
  nextReconnectMs_ = 0;

  // Abort any in-flight scan so begin() can run cleanly.
  if (scanInProgress_) {
    WiFi.scanDelete();
...
void WifiManager::pauseAutoReconnect(bool pause) {
  autoReconnectPaused_ = pause;
  if (pause)
    nextReconnectMs_ = 0;
}

bool WifiManager::autoReconnectPaused() {
  return autoReconnectPaused_;
}
...
void WifiManager::disconnectSta() {
  ...
  CaptivePortal::setActive(true);
  hasBssid_ = false;
  LOG_I("WiFi", "STA disconnected by user (auto-reconnect paused)");
}

bool WifiManager::abortStaForScan() {
  if (state_ != WifiState::Connecting && state_ != WifiState::Reconnecting &&
      state_ != WifiState::Scanning)
    return false;
  LOG_I("WiFi", "Aborting STA connect/reconnect so scan can start");
  WiFi.disconnect(false, false);
  nextReconnectMs_ = millis() + 45000UL; // give the user time to pick a network
  state_ = WifiState::ApOnly;
  return true;
}
...
bool WifiManager::startScan(bool force) {
  ...
  // ESP32 cannot start a scan while STA is mid-connect — abort first.
  abortStaForScan();
  ...
  if (rc == WIFI_SCAN_FAILED) {
    // One more hard reset of STA radio path, then retry once.
    WiFi.disconnect(false, false);
    delay(30);
  ...
  if (rc == WIFI_SCAN_FAILED) {
    ...
    SystemInfo::setLastError("Scan failed — tap Disconnect STA, then Rescan");
    return false;
  }
...
void WifiManager::loop() {
  ...
  if (state_ == WifiState::Connecting) {
    if (st == WL_CONNECTED) {
      ...
      SystemInfo::setLastError("Connect timeout — check password / Disconnect STA");
      WiFi.disconnect(false, false);
      if (settings_.autoReconnect && settings_.hasStaCreds && !autoReconnectPaused_) {
        scheduleReconnect();
        state_ = WifiState::Reconnecting;
      } else {
        state_ = WifiState::ApOnly;
...
void WifiManager::fillStatusJson(JsonObject obj) {
  ...
  obj["autoReconnectPaused"] = autoReconnectPaused_;
}
